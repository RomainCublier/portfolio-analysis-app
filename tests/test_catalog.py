...
def test_catalog_provenance_and_unknowns():
    ...
    assert sum(r["facts"]["fee_percent"] is not None for r in rows) == 6
...
def test_review_expiry_and_incomplete_are_not_verified():
    ...
    assert review_status(rows[-1], date(2026, 9, 21)) == "Fiche incomplète"
...
def test_duplicate_isins_rejected(tmp_path):
    ...
    path.write_text(json.dumps({"schema_version": 1, "instruments": [rows[0], rows[0]]}))
